...
const Recipe = () => {
  ...
  const ingredientsArray = (recipeInfo.ingredients).map((ingredient: string) => <li key={recipeInfo._id}>{ingredient}</li>);
  const instructionsArray = (recipeInfo.instructions).map((step: string) => <li key={recipeInfo._id}>{step}</li>);
      ...
      <Container style={{ display: "flex", justifyContent: "center", width: "66%" }}>
        <div>
          ...
          >
            <div>
              <Image
                src={require(`../assets/${recipeInfo.image}`)}
                width={150}
              />
            </div>
            <div>
          ...
          <Card style={{}}>
            <Stack direction={"horizontal"}>
              ...
              <div>
                ...
                {recipeInfo.prepTime + recipeInfo.cookingTime}
                {" "}
                min
              </div>
            </Stack>
